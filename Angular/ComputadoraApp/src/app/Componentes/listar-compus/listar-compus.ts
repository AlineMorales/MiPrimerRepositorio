...
export class ListarCompus implements OnInit {
  ...
  editarButon(computadora: Computadora) {
    ...
    localStorage.setItem('idCompu', computadora.idCompu.toString());
    this.router.navigate(['editar']);
  }

  eliminarButtom(compu: Computadora) {
    Swal.fire({
      title: "Seguro que quieres eliminar el id: " + compu.idCompu + "?",
      ...
      if (result.isConfirmed) {
        this.service.eliminarCompu(compu.idCompu).subscribe(data => {
          Swal.fire({
            title: "Eliminado!",
